...
std::vector<string>allstr(string s) {
	std::vector<string> v;
	int n = s.size();
	for (int i = 0; i < n; i++) {
		for (int j = 1; j <= n - i; j++) {
...
signed main()
{
	...
	wl(t--) {
		string a, b;
		cin >> a >> b;
		// std::vector<string>av = allstr(a);
		// std::vector<string>bv = allstr(b);
		int n;
		cin >> n;
		set<string>st;
		map<string, int>cost;
		...
		for (int i = 1; i <= a.size(); i++) {
			for (int j = b.size(); j >= 1; j--) {
				// string tmp = av[i] + bv[j] ;
				string sf = b.substr(0, j);
				reverse(sf.begin(), sf.end());
				string tmp = a.substr(0, i) + sf;
				// cout << tmp << el;
				int tc = 0;
				// std::vector<string> tv = allstr(tmp);
				for (auto it = cost.begin(); it != cost.end(); it++) {
					int count = kmp(tmp, it->first);
					// cout << it->first << el;
					tc += (count * it->second);
				}
				if (tc > mx) {
